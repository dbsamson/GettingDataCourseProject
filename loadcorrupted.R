...
calculateRelevantColumns <- function() {
  # setwd("C:\\Dropbox (Personal)\\Learning\\Getting and Cleaning Data\\Assignments\\Course Project\\Data")
  
  # Load measurement names(called "features" in the research)
  allDataColumns<-fread("features.txt")
  setnames(allDataColumns,c("V1","V2"),  c("columnNbr","description"))
  
  # We're supposed to only load std and mean from the main data files. Since we're using a fixed-
  # width file read function, we need to account for the fields we're skipping.  
  # Next bit calculates which fields to count and which to skip.  
  ...
  # Now create a subset of the full column list, only containing those we'll use.  
  ...
  columnNames <<- columnsToLoad$description

  # Clean up the messy column names 
  allDataColumns$description <- str_to_title(gsub("[,()]","-",allDataColumns$description))
  # allDataColumns$description <- str_replace_all(allDataColumns$description,".","")
...
activities<- mutate(activities,name=str_to_title(str_replace(name,"_"," ")))
